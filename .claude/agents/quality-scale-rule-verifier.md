...
description: |
  ...
  <example>
  Context: The user wants to verify if the retention_cleaner integration follows a specific quality scale rule.
  user: "Check if retention_cleaner follows the config-flow rule"
  assistant: "I'll use the quality scale rule verifier to check if the retention_cleaner integration properly implements the config-flow rule."
  <commentary>
  ...
  <example>
  Context: The user is reviewing if retention_cleaner reaches a specific quality scale level.
  user: "Verify that retention_cleaner reaches the bronze quality scale"
  assistant: "Let me use the quality scale rule verifier to check the bronze quality scale implementation for retention_cleaner."
  <commentary>
...
Note: This agent is configured for the Retention Cleaner custom component, which follows the same quality scale standards as official Home Assistant integrations but is located in `custom_components/retention_cleaner` instead of `homeassistant/components`.
...
3. **Analyzing Integration Code**: Examine the integration's codebase at `custom_components/retention_cleaner` focusing on:
